...
struct SegmentTree {
	...
	ll query(int pos, int l, int r, int x, int ql) {
		// p2(l,r);
		...
		int left = -1;
		if(seg[2*pos+1] >= x and ql <= mid) left = query(2*pos+1,l,mid,x,ql);
		if(left != -1) return left;
		return query(2*pos+2,mid+1,r,x,ql);
	}

	ll query(int x, int ql, int n) {
		return query(0,0,n-1,x,ql);
	}
};
