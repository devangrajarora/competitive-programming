...
struct SegmentTree {
	...
	void build(vi &a, int pos, int l, int r) {
		...
		if(seg[lc] == seg[rc]) cnt[pos] = cnt[lc] + cnt[rc];
		else if(seg[lc] < seg[rc]) cnt[pos] = cnt[lc];
		else cnt[pos] = cnt[rc];
	...
	pii query(int pos, int l, int r, int ql, int qr) {
		...
		if(ql > r or qr < l) return {LLONG_MAX,LLONG_MAX};
		if(ql <= l and r <= qr) return {seg[pos],cnt[pos]};
		...
		if(left.fi == right.fi) return {left.fi, left.se + right.se};
		else if(left.fi < right.fi) return left;
		else return right;
	}

	pii query(int ql, int qr, int n) {
